...
#define CLONE_PIDFD 0x00001000
	...
	pid_t runChild(nsjconf_t *nsjconf, int netfd, int fd_in, int fd_out, int fd_err)
	{
		...
		LOG_I("Created new process with pid=%d", pid);

		// use pipe to send pid
		// establish a named pipe: nsjconf->cmd_pipe
		// write pid to pipe

		int pipefd = open(nsjconf->cmd_pipe.c_str(), O_WRONLY);
		if (pipefd == -1)
		{
			PLOG_E("open(%s, O_WRONLY)", nsjconf->cmd_pipe.c_str());
			return -1;
		}

		const char *pid_c_str = ("pid:" + std::to_string(pid)).c_str();
		if (!util::writeToFd(pipefd, pid_c_str, strlen(pid_c_str)))
		{
			LOG_E("Couldn't write to the pipe");
			close(pipefd);
			return -1;
		}
		else
		{
			LOG_I("Wrote pid to pipe: %s", pid_c_str);
			close(pipefd);
		}
